...
def time_2(vid):
    ...
    elbow_sorted = sorted(elbow_x)
    maxes = elbow_sorted[:10]

    elbow_x = list(elbow_x)
    for x in range(9):
      max_indx = elbow_x.index(maxes[x])  
      max_loc.append(max_indx)

    max_loc = list(max_loc)
    t2 = statistics.median(max_loc)
    vid.t2 = int(t2)
...
def time_1(vid):
    ...
    wrist_x = a.wrist_x
    t2=vid.t2
    #smoothing with exponential filtering
    wrist_x = wrist_x[:t2]
    count = len(wrist_x)
    sm1 = wrist_x[0]
    smooth = [sm1]
    ...
    for x in range(1,count):
      new = (0.1*wrist_x[x]) + (0.9*smooth[x-1])
      smooth.append(new)
